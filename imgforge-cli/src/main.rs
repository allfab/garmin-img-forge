...
fn read_mp_file(path: impl AsRef<Path>) -> Result<String> {
    ...
    match String::from_utf8(bytes.clone()) {
        ...
        Err(_) => {
            ...
            Ok(bytes.iter().map(|&b| cp1252_byte_to_char(b)).collect())
        }
    }
}

/// Map a CP1252 byte to its Unicode character.
/// 0x00-0x7F and 0xA0-0xFF map directly, 0x80-0x9F have special mappings.
