# Types Garmin — Référence complète BDTOPO

Les codes types (`Type=0xNNNN`) déterminent comment chaque objet est rendu sur les appareils Garmin. Cette page documente l'ensemble des types utilisés dans le mapping BDTOPO → Garmin IMG.

Les codes `0x0001`–`0x00FF` sont des types Garmin standard. Les codes `0x10000`–`0x1FFFF` sont des types personnalisés qui nécessitent un fichier TYP (`.typ`) pour définir leur rendu visuel.
...
Note : la classification par taille de lac (`0x3c` → `0x41`) dépend de la surface géométrique du polygone.

### Détails hydrographiques (POI)
...
## Pylônes et constructions ponctuelles
