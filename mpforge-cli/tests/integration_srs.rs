...
use mpforge_cli::config::{Config, InputSource};
...
fn make_config(input: InputSource) -> Config {
    Config {
        version: 1,
        grid: mpforge_cli::config::GridConfig {
            cell_size: 10.0, // Large cell to cover all test data
            overlap: 0.0,
            origin: None,
        },
        inputs: vec![input],
        output: mpforge_cli::config::OutputConfig {
...
fn test_srs_explicit_source_and_target_reprojection() {
    ...
    let extent = SourceReader::scan_extents(&config).unwrap();
    let tile_bounds = mpforge_cli::pipeline::tiler::TileBounds {
