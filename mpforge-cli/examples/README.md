# Exemples de configuration mpforge-cli
...
```bash
# Depuis le répertoire mpforge-cli
mpforge-cli build --config examples/simple.yaml
```
...
mpforge-cli build \
  --config /home/allfab/code/forgejo/mpforge/mpforge-cli/examples/france-nord-bdtopo.yaml \
  --jobs 4 \
...
mpforge-cli build \
  --config /home/allfab/code/forgejo/mpforge/mpforge-cli/examples/france-nord-simple.yaml \
  --jobs 8
```
...
cd /home/allfab/code/forgejo/mpforge/mpforge-cli/examples/
...
- ✅ Vérification de l'environnement (mpforge-cli, données)
...
mpforge-cli build --config config.yaml --jobs $(nproc)
```
...
mpforge-cli build --config config.yaml -vv

# Logs ultra-verbeux (trace)
mpforge-cli build --config config.yaml -vvv
```
...
mpforge-cli validate --config config.yaml
```
