...
fn test_version_flag_short() {
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::starts_with("mpforge "));
}
...
fn test_version_format() {
    ...
    // Format attendu: "mpforge <version>" (git describe ou CARGO_PKG_VERSION)
    assert!(
        stdout.starts_with("mpforge "),
        "Version output should start with 'mpforge ', got: {}",
        stdout
    );
