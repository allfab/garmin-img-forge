...
pub fn run_validate(
    ...
    // Step 6: Spatial filter source files (optional, per-input)
    {
        ...
        if sf_sources.is_empty() {
            ...
            // Grouper par source unique pour éviter de répéter N fois le même fichier
            // (ex : COMMUNE.shp utilisé comme spatial_filter sur 83 couches).
            // order_keys préserve l'ordre de première apparition ; by_source = lookup O(1).
...
"#;
    ...
    #[test]
    fn test_profile_validates_simplify_bound_detail() {
        // n=0 (palier détail) conserve la borne stricte 0.001 — 0.01 dépasse
        // et doit être rejeté (anti-destruction sur la zone détaillée).
        let p = parse_profile("levels:\n  - { n: 0, simplify: 0.01 }\n");
        let err = p.validate("test").unwrap_err().to_string();
